...
public class EmbeddedServletContainerPortRangeConfig {
	...
	/**
	 * Sourced from application.properties / application.yml: serverPortRange.max
	 * <p/>
	...
	/**
	 * Sourced from application.properties / application.yml: serverPortRange.min
	 * <p/>
	...
	private void validatePorts() {
		...
		if (min < MIN_PORT || min > MAX_PORT - 1) {
			throw new IllegalStateException("Minimum port range value must be between " + MIN_PORT + " and " + (MAX_PORT - 1) + ".");
		}

		if (max < MIN_PORT || max > MAX_PORT) {
			throw new IllegalStateException("Maximum port range value must be between " + (MIN_PORT + 1) + " and " + MAX_PORT + ".");
		}
